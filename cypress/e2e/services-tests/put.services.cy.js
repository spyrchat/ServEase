...
describe("Servease app: PUT /services/{serviceId}", () => {
  ...
  it("Contains PUT /services/{serviceId}", () => {
    cy.get("#operations-services-editService")
      ...
      .within(() => {
        ...
        cy.contains(".opblock-summary-description", "Edit a service").should(
          "exist"
        );
  ...
  /**
   * Checks if PUT /services/{serviceId} endpoint is clickable and displays additional details
   */
  it("PUT /services/{serviceId} is clickable and appears right", () => {
    cy.contains(".opblock-summary-description", "Edit a service").click(); // Clicks on the PUT /services/{serviceId} summary to expand it
  ...
  /**
   * Verifies if the tryout functionality for PUT /services/{serviceId} works and returns a valid response
   */
  it("PUT /services/{serviceId} tryout works and returns 200", () => {
    // Load the JSON data from the fixture
    cy.fixture("service").then((service) => {
      // Expand the PUT /services/{serviceId} section
      cy.contains(".opblock-summary-description", "Edit a service").click();
      ...
      cy.get('input[placeholder="serviceId - The service\'s id"]').type("1");

      // Insert the entire JSON as a string
      cy.get(".body-param .body-param__text")
        .clear()
        .type(JSON.stringify(service, null, 2), {
      ...
      cy.get(
        "#operations-services-editService .responses-table.live-responses-table"
      )
