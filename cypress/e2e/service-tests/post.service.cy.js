...
describe("Servease app: POST /services", () => {
    ...
    it("POST /services is clickable and appears right", () => {
      ...
      cy.get("#operations-services-createService").should("have.class", "is-open"); // Confirms the section is expanded
    ...
    it("POST /services tryout works and returns 200", () => {
      ...
      cy.fixture("newService").then((newService) => {
        ...
        cy.get(".body-param .body-param__text")
          .clear()
          .type(JSON.stringify(newService, null, 2), { parseSpecialCharSequences: false });
  
        // Execute the POST request
        cy.get(".btn.execute.opblock-control__btn").click();
  
        cy.get("#operations-services-createService .responses-table.live-responses-table")
